...
try:
    ...
    # Keep the standalone PyMOL script usable even when only this file is
    # downloaded outside of the full APPRAISE package.
    discover_structure_files = None
    extract_appraise_job_name = None
    parse_appraise_job_name = None
...
def generate_pdb_path_list(AF2_results_path, use_relaxed=use_relaxed_global):
    """
    Get a list of structure file paths from APPRAISE-compatible modeling
    results.
    """
    if discover_structure_files is not None:
        return discover_structure_files(AF2_results_path, use_relaxed=use_relaxed)
...
def parse_pdb_file_name(pdb_path):
    """
    Parse out the peptide name and receptor name from structure file names.
    """
    if parse_appraise_job_name is not None:
        return parse_appraise_job_name(pdb_path)
...
def quantify_peptide_binding_in_pdb(pairwise_mode=True, \
    ...
    The main function to measure different metrics for quantifying
        peptide-receptor binding within a single structure file.
    ...
    if extract_appraise_job_name is not None:
        model_name = extract_appraise_job_name(pdb_path)
...
def quantify_results_folder(AF2_results_path='./*result*/', \
    ...
    """
    AF2_results_path: (str) path to the folder(s) that contain the modeling
    results. Wildcard is allowed. Default: all folders containing "results" in
    their name under the current working directory.
        # Format requirement of the structure file (files generated by the
        # pipeline should already meet the requirements):
        # Filname or containing folder should preserve the APPRAISE job name,
        # e.g. ReceptorName_and_Peptide1Name_vs_Peptide2Name.
        # The structure file should have more than 1 chains (('A'), 'B', 'C'...),
    ...
    # list out all structure files in the folder
